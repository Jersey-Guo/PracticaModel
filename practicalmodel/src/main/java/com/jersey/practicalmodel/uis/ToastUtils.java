package com.jersey.practicalmodel.uis;
...
public final class ToastUtils {
    ...
    private static final int BG_DEFAULT = 0xFEFFFFFF;
    ...
    private static int bgColor = BG_DEFAULT;
     ...
     * @param text 文本
     */
    public static void showWhiteShort(@NonNull final CharSequence text) {
        showWhite(text, Toast.LENGTH_SHORT);
    }
     ...
     * 安全地显示短时自定义吐司
     */
    public static View showCustomShort(@LayoutRes final int layoutId, final int textViewId, final String text) {
        final View view = getView(layoutId);
        final TextView tv = (TextView) view.findViewById(textViewId);
        tv.setText(text);
        showNoBg(view, Toast.LENGTH_SHORT);
        return view;
    }
    ...
    private static void showWhite(final CharSequence text, final int duration) {
        HANDLER.post(new Runnable() {
            @Override
            public void run() {
                cancel();
                ...
                background.setColorFilter(new PorterDuffColorFilter(0xff64a4ff, PorterDuff.Mode.SRC_IN));
    ...
    private static void showNoBg(final View view, final int duration) {
        HANDLER.post(new Runnable() {
            @Override
            public void run() {
                cancel();
